...
export const addCharacter = createAsyncThunk(
  "character/add",
  async ({ name, description, prompt, scenario, first_mes, mes_example, relationship, appearance, appearanceImages, avatar, accent, tags, voiceURI, autoSelfie }: Omit<Character, "id">, { rejectWithValue }) => {
    try {
      if (!name.trim() || !prompt.trim()) {
        throw new Error("Character name and prompt are required.");
      }
      const newChar = { name, description, prompt, scenario, first_mes, mes_example, relationship, appearance, appearanceImages, avatar, accent, tags, voiceURI, autoSelfie };
